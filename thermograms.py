...
SINE_PALETTE = lambda waves: np.repeat(
    (
        255 * (np.sin(np.linspace(0, 2*np.pi*waves, 257))/2 + 0.5)
    ).astype('uint8').reshape(-1, 1),
    3,
    axis=1
)
...
class Timage:
    ...
    def show(self, palette=WB_PALETTE) -> np.ndarray:
        """Show thermogram"""

        image = self.array
        image -= image.min()
        image /= image.max() if image.max() > 0.0 else 1.0 # if image.max() == 0 then all timage is made of one value
        image = (255*image).astype('uint8')

        colored = np.take(palette, image, axis=0) # colored[i,j] = palette[image[i,j]]
    ...
    def median_blur(self, radius=3) -> "Timage":
        ...
        out = median_filter(self.__arr.astype('float32'), size=(2*radius+1, 2*radius+1), mode='reflect')
...
class Tseries:
    ...
    def median_blur(self, radius=3) -> "Tseries":
        ...
        for i in trange(new_arr.shape[2]):
            new_arr[:, :, i] = median_filter(self.__arr[:, :, i].astype('float32'), size=(2*radius+1, 2*radius+1), mode='reflect')
    ...
    def std_map(self, nd: str | Tuple[int] = 'avg', binarization: str | float = 'otsu') -> np.ndarray:
        ...
        return (diff>=threshold).copy().astype(np.float16)
